...
float4 Cloud(float3 bgCol,float3 ro,float3 rd,float3 cloudCol,float spd, float layer)
{
	float4 col = float4(bgCol.x, bgCol.y, bgCol.z, 0);
    float time = _Time.y*0.05*spd;
	float2 sc;
	for(int i=0; i<layer; i++){
		sc =ro.xz + rd.xz*((i+3)*40000.0-ro.y)/ rd.y;
		col.xyz = lerp( col, cloudCol, 0.5*smoothstep(0.5,0.8,TimeFBM(0.00002*sc,time*(i+3))) );
		col.w = sc.x;
	}
	return col;
}
