...
#include"Lighting.cginc"
		...
		fixed4 frag(v2f i) :SV_Target{
			...
			//float currentTexCoords = i.uv.zw;

			//float currentDepthMap = tex2D(_DepthMap, currentTexCoords).r;
			//for (float x = 0; x < 64; x++)
			//{
			//	if (currentLayer < currentDepthMap)
			//	{
			//		currentTexCoords -= deltaTexcoords;
			//		currentDepthMap = tex2D(_DepthMap, currentTexCoords).r;
			//		currentLayer += layersDepth;
