...
Shader "FishManShaderTutorial/Line"{
	Properties{
...
//-----------------------------------------------------
			...
			fixed4 ProcessFrag(v2f i)  {
				...
				finalColor = fixed3(cro,cro,cro  );
				finalColor += ( circle(uv, c, 0.5, .01)
							  + circle(uv, c, 0.45, .01 ))* blue1;
				finalColor += (circle(uv, c, 0.4, .01) );//+ dots(uv,c,240.0)) * blue4;
				finalColor += circle3(uv, c, 0.35, 0.01) * blue1;
				finalColor += triangles(uv, c, 0.3+ sin(_Time.y)) * blue2;
				finalColor += movingLine(uv, c, .5) * blue3;
				finalColor += circle(uv, c, .2,.01) * blue3;
				finalColor += 0.7 * circle2(uv, c, 0.25, .01, 0.5+0.2*cos(_Time.y)) * blue3;
				if( length(uv-c) < 0.25 )
				{
