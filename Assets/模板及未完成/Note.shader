		...
		SubShader
	{
		...
		v2f vert(appdata v)
		{
			...
			//光源类型 平行光：位置固定、衰减固定为1 点光源：有位置、衰减由一个函数定义 聚光灯：位置、范围、衰减
				...
				//Diffuse光 _LightColor0 平行光 lambert
				fixed3 diffuse = _LightColor0.rgb * /*(表面颜色) * */ max(0, dot(worldnormal, worldLightDir));
				//Specular光  
				fixed3 specular = _LightColor0.rgb */*(高亮颜色) * */ pow(max(0, dot(worldnormal, halfDir)), _Gloss);
				//Specular光 halfDir
				float halflight = _LightColor0.rgb * _Specular.rgb *pow(saturate(dot(halfDir, worldLightDir)), _Gloss);
				//标准Blinn
				//r =  2(n . l)* n -l
				fixed3 reflctdir = 	2*(dot(worldnormal,worldLightDir)) * worldnormal - worldLightDir;
				fixe3 blinn = _LightColor0.rgb * /*(高亮颜色) * */ pow(max(0,dot( viewDir,reflctdir)),_Gloss);
				...
				//水0.02 玻璃0.03 塑料 0.05 宝石 0.08 钻石 0.17 铁 0.56 铜 0.64 金 1  
				float fresnel = 0.02 + (1-0.02)*pow(1 - dot(worldnormal, viewDir), 5);
				//col.rgb += lerp(col.rgb, _fresnelCol, fresnel) * _fresnelCol.a;
				
				//金属性
				//(1- F ) *(1- Metalic) * diffuse

				//DGF D:法线分布函数(高光反射) G几何函数(粗糙度 1max) F 相加
