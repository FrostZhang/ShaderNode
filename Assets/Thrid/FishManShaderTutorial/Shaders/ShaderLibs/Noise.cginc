...
fixed3 mod289(fixed3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
fixed2 mod289(fixed2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
fixed3 permute(fixed3 x) { return mod289(((x*34.0)+1.0)*x); }
float RainNoise(fixed2 v) {
    ...
    fixed3 p = permute( permute(  fixed3(i.y , i1.y+i.y , 1.0 +i.y ))+
         fixed3(i.x , i1.x+i.x , 1.0+i.x  ));
