...
v2f vert(appdata_img v) {
    ...
    int index = 0;
 //	int x = step(v.texcoord.x, 0.5) *1 ;
	//int y = step(v.texcoord.y, 0.5) *2;
	//index = x + y;
...
//uv:_Screenuv ro:_WorldSpaceCameraPos rd:cameraray depth:unitydepth sceneCol:_MainTex
float4 ProcessRayMarch(float2 uv,float3 ro,float3 rd,inout float sceneDep,float4 sceneCol);

float4 frag(v2f i) : SV_Target{
    float depth = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv_depth));
    depth *= length(i.interpolatedRay.xyz);  //max: camera Far
