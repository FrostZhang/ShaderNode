...
//-----------------------------------------------------
			...
			fixed4 ProcessFrag(v2f i)  {
				...
				finalColor += ( circle(uv, c, 0.49, _W)
							  + circle(uv, c, 0.485, _W))* green1;
				finalColor += (circle(uv, c, 0.4, _W) )* green2;//+ dots(uv,c,240.0)) * blue4;
				finalColor += circle3(uv, c, 0.32, _W) * green1;
				//finalColor += triangles(uv, c, 0.3+ sin(_Time.y)) * blue2;
				finalColor += movingLine(uv, c, .5) * green3;
				finalColor += circle(uv, c, .15, _W) * green3;
				finalColor += 0.7 * circle2(uv, c, 0.24, _W, 0.5+0.2*cos(_Time.z)) * green3;
				//if( length(uv-c) < 0.25 )
				//{
